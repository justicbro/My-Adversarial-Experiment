...
def objective_function_ATTR(X, mask, G_list, E, iner_num_steps = 100, inner_tol = 1e-10, my_device="cuda"):
    ...
    # TN_G2 = tl.tr_to_tensor(tr_cores)  # Reconstruct tensor from tensor train
    # 确保 tr_cores 启用了梯度追踪
...
def optimize_perturbation(X, mask, G_list_ori, config: OptimizationConfig):
    """
    ...
    with torch.no_grad():
        E[mask == 0] = 0  # Set perturbation to zero where mask is zero
    # Apply the constraint ||E||_F^2 <= epsilon
    scaling_factor = (config.epsilon ** 0.5) / torch.norm(E)
    if torch.norm(E) ** 2 > config.epsilon:
        E.data = scaling_factor * E
    ...
    for step in range(config.outer_num_steps):
        ...
        # Compute loss
        loss, TN_G, G_list = objective_function_ATTR(X, mask, G_list_ori, E, config.inner_num_steps, config.inner_tol, config.my_device)
        ...
        rse_E = torch.norm(E - E_old).item()

        loss_X_TNG = torch.norm((X - TN_G))
        loss_X_TNG = loss_X_TNG.cpu().detach().numpy()
        loss_list.append(loss_X_TNG)

        norm_E = torch.norm(E).item()
        ...
        # X.requires_grad_(True)
        # E = E.detach()  # 分离计算图
        # E.requires_grad_(True)  # 重新启用梯度跟踪
        # # TN_G[mask == 1] = X[mask == 1]
        # TN_G = TN_G.detach()
        # G_list = [g.detach() for g in G_list]
        # G_list_ori = [g.detach() for g in G_list_ori]
        # mask = mask.detach()

        # 释放计算图
        # del loss, G_list, E_old
        # gc.collect()
        # torch.cuda.empty_cache()
...
if __name__ == "__main__":
    ...
    tr_cores = tr.tr_decompose(X*mask, ranks, max_iter=100, tol=1e-10, device="cuda")
    tr_cores = [tr_core.requires_grad_(True) for tr_core in tr_cores]

    # Set input parameter 
    epsilon = (5e-1)**2 # Frobenius norm constraint for perturbation tensor
    ...
    outer_num_steps = 500 # Number of outer optimization steps
    ...
    TN_G, tr_cores_ours = tr.tensor_completion(X+E_optimized, mask, tr_cores, max_iter=inner_num_steps, tol=inner_tol, dis_num=inner_dis_num, device="cuda")
    ...
    TN_G_ori, tr_cores = tr.tensor_completion(X, mask, tr_cores, max_iter=inner_num_steps, tol=inner_tol, dis_num=inner_dis_num, device="cuda")
